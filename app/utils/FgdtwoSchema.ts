...
name_of_facility_provider:object().shape({
  text: string().when(['$permanent_residency','reason'], ([p_r,r], schema:any) =>{
    ...
    return (p_r && parseInt(p_r.value) < 2) && (JSON.stringify(r) === '{}')?
    schema.required('text is required!'):schema;
  }),
...
treatment_location_last_year:object().shape({
  value: string(),
  label: string()
}).when(['$permanent_residency','$name_of_facility_provider','$treatment_location_person_type'],{  
is:(p_r:any, n_f_p:any,tf_t_l_p_type:any)=>{
   const l:any = tf_t_l_p_type.filter((item:any) => ![20,21,22,23,888,999].includes(parseInt(item.value))).length;
   const check = (p_r && (parseInt(p_r.value) < 2 )) && (n_f_p && n_f_p.value =='') &&  ( l > 0) ;
   return check
},
  then: (schema:any) =>{
           return schema.shape({
...
waiting_time_meet_doctor:object().shape({
  hour: number().when(['$treatment_location_last_year','reason'], ([t_r_y,r], schema:any) =>{
    //console.log('reason',r)
    return (t_r_y&& parseInt(t_r_y.value) < 2) && (JSON.stringify(r) === '{}')?
    schema.min(0,'hour at least 0').max(24,'hour allow max 24').required('hour is required!'):schema;
  }),

minute: number().when(['$treatment_location_last_year','reason'], ([t_r_y,r], schema:any) =>{
    //console.log('reason',r)
    return (t_r_y&& parseInt(t_r_y.value) < 2) && (JSON.stringify(r) === '{}')?
    schema.min(0, 'minute at least 0').max(60,'minute allow max 60').required('minute is required!'):schema;
  }),

  reason: object({
    value:string(),
    label:string()
  }).when(['$treatment_location_last_year','hour','minute'], ([t_r_y,h,m], schema:any) =>{
    //console.log('text',t)
    return (t_r_y&& parseInt(t_r_y.value) < 2) && ((typeof h == 'undefined') || (typeof m == 'undefined'))?
    schema.shape({
      value:string().required('denied reason is required!'),
      label:string().required('denied reason is required!')
    }):schema
...
minute: number().when(['$treatment_location_last_year','reason'], ([t_r_y,r], schema:any) =>{
  ...
  reason: object({
    value:string(),
    label:string()
  }).when(['$treatment_location_last_year','minute'], ([t_r_y,m], schema:any) =>{
    //console.log('text',t)
    return (t_r_y&& parseInt(t_r_y.value) < 2) && (typeof m == 'undefined')?
    schema.shape({
      value:string().required('denied reason is required!'),
      label:string().required('denied reason is required!')
    }):schema
