...
const BillGrid = ({ category }: BillProps) => {
  ...
  const { error, loading, totalRows, pageCount } = useFetchGridData({
    urlSuffix: `billspg/${authCtx.userId}${
            ...
            onClick: async () => {
              DeleteActionGlobal({
                ...
                if (deletedData.deleted_done > 0) {
                  ...
                  const updatedData: DataRow[] = data.filter(
                    (row: DataRow) => row.id !== id
                  );
    ...
    () => [
...

      {
        ...
        cell: (info) => (
          <p>
            <Link
              className="text-[#43ACD6]"
              href={`bills/${info.row.getValue("id")}`}
            >
              {info.getValue()}
            </Link>
          </p>
...

      {
        ...
        cell: (info) => (
          <p>
            {info.row.original.bill_type_parent &&
              `${info.row.original.bill_type_parent} > `}
            {info.getValue()}{" "}
          </p>
        ),
