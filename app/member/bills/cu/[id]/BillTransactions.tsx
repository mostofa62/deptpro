...
const BillTransactions = ({
  ...
  const { error, loading, totalRows, pageCount } = useFetchGridData({
    urlSuffix: `bill-transpg/${bill_acc_id}`,
  ...
  const fetchPayments = useCallback(async (id: number) => {
    //alert(id)
    //console.log(id);
    const response = await axios.get(`${url}bill-paymentspg/${id}`);
  ...
  return (
    <div className="flex flex-col">
      <p className="text-sm text-center md:text-left lmd:text-left md:text-[16px] uppercase font-medium mt-1 md:mt-3">
        Bill History
      </p>

      <div className="flex flex-wrap gap-1 mt-2 md:mt-4">
        {loading ? (
          <div className="w-full col-span-2 justify-center items-center md:mt-[-100px]">
            <Loading width={150} />
          </div>
        ) : (
          <>
                    ...
                    <div className="flex flex-col gap-1 p-2 md:p-0 lmd:p-0">
                      <div className="flex">
                        <div className="w-[50%] text-left">
                          <p className="text-[15px] font-semibold flex gap-2.5">
                            <span>Balance</span>
                            <span>
                              $
                              ...
                              }).format(row.original.current_amount)}{" "}
                            </span>
                          </p>
                          <p className="text-[14px] font-semibold flex gap-2.5">
                            <span>Amount</span>
                            <span>
                              $
                              {Intl.NumberFormat("en-US", {
                        ...
                        <div className="w-[50%] flex flex-row justify-end items-end">
                          {row.original.payment_status > 0 ? (
                            <svg
                              className="text-[#31c4a2] font-semibold"
                              width={22}
                              height={22}
                      ...
                      <div className="flex">
                        <div className="w-full col-span-2">
                          <p className="font-medium text-[15px]">
                            <span>DUE:</span>
                            <span className="ml-1">
                              {moment(row.original.due_date).format(
                                "MMMM DD, YYYY"
                              )}
                            </span>
                          </p>
                        ...
                        <div className="flex">
                          <div className="w-[50%]">
                            <button
                              ...
                              className="text-[15px] w-full h-[40px] bg-[#56595e] rounded text-white px-4  capitalize text-center font-semibold"
                            >
                              Add Payment
                            </button>
                          </div>
                          <div className="w-[50%] flex flex-row justify-end items-end">
                            ...
                            <button
                              ...
                              title="Delete"
                              className="ml-5 text-[15px] h-[38px] border-[#56595e] border  rounded text-[#56595e] px-2  capitalize text-center font-semibold"
                            >
                              <svg
                                width={18}
                                height={18}
                      ...
                      {row.original.payment_status > 0 && (
                        <div className="flex justify-center">
                          <p
                            onClick={async () => fetchPayments(row.original.id)}
                      ...
                      {payments.trans_id == row.original.id &&
                        payments.payments.length > 0 && (
                          <div className="mt-5">
                            {isMobile || isTab ? (
                              <div className="flex flex-col gap-2">
                                {payments.payments.map(
                                  (data: paymentRow, index: number) => {
                                    return (
                                      <div
                                        className="flex flex-col gap-1 rounded border shadow-sm p-2"
                                        key={index}
                                      >
