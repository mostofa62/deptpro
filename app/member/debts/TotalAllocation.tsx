...
const TotalAllocation = ({ userid }: TotapProps) => {
      ...
      <div className="bg-white border p-2 rounded shadow-lg text-sm">
        ...
        {payload.map((entry: any, index: number) => (
          <div key={`item-${index}`} style={{ color: entry.stroke }}>
            <strong>{debt_type_names[entry.dataKey]}:</strong> $
            {Intl.NumberFormat("en-US", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            }).format(entry.value)}
    ...
    if (total_length > 0) {
      const heights =
        itemRefs?.current && itemRefs.current.length > 0
          ...
          : [];
      const tallestHeight = heights.length > 0 ? Math.max(...heights) : 450;
  ...
  const ids = data.length > 0 ? data.map((item: any) => item.id) : [];

  const uniquecolors = ids.length > 0 ? generateUniqueColors(ids) : {};

  const debt_type_keys = Object.keys(debt_type_names);

  const uniquecolors_debt =
    debt_type_keys.length > 0 ? generateUniqueColors(debt_type_keys) : {};
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-2.5">
      ...
      >
        {data.length > 0 && Object.keys(uniquecolors).length > 0 && (
          <CardHolder title="Total Allocation" maxHeight={maxHeight}>
            <div className="flex flex-col  md:flex-row">
              {/* {JSON.stringify(data)} */}
              <div className="w-full md:w-[45%] flex items-center justify-center">
                <PieChart
                  width={isMobile || isTab ? 150 : 250}
                  height={isMobile || isTab ? 150 : 250}
                >
                  <Pie
                    data={data}
      ...
      <div
        ...
        style={{ height: maxHeight ? `${maxHeight}px` : "auto" }}
      >
        {chartData.length > 0 && Object.keys(uniquecolors_debt).length > 0 ? (
          <CardHolder title="12 Months Projection" maxHeight={maxHeight}>
            <div className="w-full overflow-x-auto">
              {" "}
              {/* Scrollable container */}
              <div className={`w-[${chartData.length * 100}px]`}>
                {" "}
