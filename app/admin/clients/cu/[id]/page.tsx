...
export default function InsuranceCreate({
  ...
  const fetchDataCallback = useCallback(async () => {
    ...
    const role = UserRoles.find(
      (item: any) => item.value === response.data.user.role
    );
    console.log("role", role);
    setFetchFormData({ id, ...response.data.user, role });
  }, [id]);
  useEffect(() => {
    fetchDataCallback();
  }, [fetchDataCallback]);
  ...
  const handleFormSubmit = async (values: any, { resetForm }: any) => {
    ...
    await axios
      .post(
        `${url}user-createpg/${id}`,
        { ...values.fetchdata, role: values.fetchdata.role.value },
        {
          headers: {
    ...
    <>
      <AdminLayout>
        <div className="flex flex-col">
          <HolderOne title="clients" linkItems={linkItems} />
              ...
              render={({
                ...
                <FormikFormHolder>
                  ...
                  <div className="flex">
                    <div className="w-[50%]">
                      <FormikSelectInput
                        label={DataLabel.role}
                        defaultValue={fetchdata.role}
                        placeHolder={`Select ${DataLabel.role}`}
                        isSearchable={true}
                        isClearable={true}
                        name="fetchdata.role"
                        ...
                        errorMessage={
                          errors.fetchdata &&
                          errors.fetchdata.role &&
                          touched.fetchdata &&
                          touched.fetchdata.role &&
                          errors.fetchdata.role.label
                        }
                      />
                    </div>
                  </div>
