...
type ExtraDayDataItem = {
  ...
  const AdvancedCalendar: React.FC<AdvancedCalendarProps> = ({ extraDayData,currentMonth,onChangeMonth }) => {
  ...
  const renderDates = () => {
    ...
    return daysArray.map((day, index) => {
      ...
      const extraDataArray = Array.isArray(extraDayData)
        ...
        : (extraDayData[formattedDate] ? [extraDayData[formattedDate]] : []);
  
      return (
        <div
          key={index}
          className={`h-16 min-h-[4rem] w-auto flex flex-col items-center justify-center relative rounded-lg  group ${
            extraDataArray.length > 0 ? 'bg-[#31c4a2] text-white' : 'bg-gray-200 text-black'
          }`}
        >
          ...
          {extraDataArray.length > 0 && (
            ...
            >
              {extraDataArray.map((extra, idx) => (
                <div key={idx} className={`p-2 text-xs text-[#4f4f4f] ${idx+1 < extraDataArray.length ? 'border-b-[1px] border-[#bcbaba]':''}`}>
                  <div className="font-semibold text-sm cursor-pointer">{extra.title}</div>
